...
class content extends admin {
    ...
    public function uploads() {
        if(isset($_POST['dosubmit'])) {
            ...
            $modelid = $this->categorys[$catid]['modelid'];
            $this->db->set_model($modelid);
            $table_name = $this->db->db_tablepre.$this->model[$modelid]['tablename'];
            foreach ($_POST['pic_alt'] as $key => $value) {
                $this->db->table_name = $table_name;
                $info['catid'] = $catid;
                $info['typeid'] = 0;
                $info['title'] = $value;
                $info['thumb'] = $_POST['pic_url'][$key];
                ...
                $rid = $this->db->insert($info,true);
                $urls = $this->url->show($rid, 0, $catid);
                $data['url'] = $urls[0];
                $this->db->update($data,array('id'=>$rid));
                $info2['id'] = $rid;
                $info2['content'] = $value;
                $info2['readpoint'] = 0;
                $info2['allow_comment'] = 1;
                if($this->model[$modelid]['tablename'] == 'product') {
                    $arr[0]['url'] = $_POST['pic_url'][$key];
                    $arr[0]['alt'] = $value;
                    $info2['pics'] = array2string($arr);
                }
                $this->db->table_name = $table_name.'_data';
                $this->db->insert($info2);
            }
            showmessage(L('add_success'),$forward);
        } else {
            include $this->admin_tpl('content_all_upload');
        }
    }
}
?>