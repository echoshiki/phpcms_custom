...
class category extends admin {
	...
	public function init () {
		...
		if(!empty($result)) {
			foreach($result as $r) {
				// 筛选模型栏目
				if (isset($_GET['modelid']) && intval($_GET['modelid']) !== intval($r['modelid'])) { continue; }
				...
				if($r['type']==0 ){
					$r['str_manage'] .= ' | <a href="?m=content&c=content&a=init&catid='.$r['catid'].'&menuid='.$_GET['menuid'].'&s='.$r['type'].'&pc_hash='.$_SESSION['pc_hash'].'" >'.'内容管理'.'</a>';
				}else{
					$r['str_manage'] .= ' | <a href="?m=content&c=content&a=add&catid='.$r['catid'].'&menuid='.$_GET['menuid'].'&s='.$r['type'].'&pc_hash='.$_SESSION['pc_hash'].'" >'.'内容管理'.'</a>';
				}
