	...
	public function edit() {
		...
		if ($data = $this->db->get_one(array('siteid'=>$siteid))) {
			if (isset($_POST['dosubmit'])) {
				...
				// 新加站点信息配置
				$company_cn = trim($_POST['company_cn']);
				...
				$setting = trim(array2string($_POST['setting']));
				// 添加新字段
				$sql = array('name'=>$name,'dirname'=>$dirname, 'domain'=>$domain, 'site_title'=>$site_title, 'keywords'=>$keywords, 'description'=>$description, 'release_point'=>$release_point, 'template'=>$template, 'setting'=>$setting, 'default_style'=>$default_style, 'company_cn'=>$company_cn, 'company_en'=>$company_en, 'contact'=>$contact, 'qq'=>$qq, 'mobile'=>$mobile, 'tel'=>$tel, 'email'=>$email, 'address'=>$address, 'mappoint'=>$mappoint, 'zipcode'=>$zipcode, 'icp'=>$icp);
